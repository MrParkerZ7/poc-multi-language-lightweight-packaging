# 1-optimize-spring-native: Spring Boot 3 + GraalVM native image (keeps Spring, AOT-compiled)
# Requires: GraalVM 21 with native-image (run `gu install native-image`)
$ErrorActionPreference = "Stop"

Write-Host "[1-optimize-spring-native] mvn -Pnative native:compile -DskipTests" -ForegroundColor Cyan
