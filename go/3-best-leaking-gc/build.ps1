$13-best-leaking-gc (Go): TinyGo + gc=leaking + scheduler=none + panic=trap + opt=z + UPX --best --lzma
...
Write-Host "[3-best-leaking-gc] tinygo build -opt=z -no-debug -gc=leaking -scheduler=none -panic=trap" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[3-best-leaking-gc] upx --best --lzma app.exe" -ForegroundColor Cyan
