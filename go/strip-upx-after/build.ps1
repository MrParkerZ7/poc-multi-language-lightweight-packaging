# strip-upx-after: stripped + UPX-compressed
# Requires: UPX on PATH (https://upx.github.io/)
$ErrorActionPreference = "Stop"

Write-Host "[strip-upx-after] go build -ldflags='-s -w' -trimpath" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[strip-upx-after] upx --best --lzma app.exe" -ForegroundColor Cyan
