# after-strip-upx: stripped + UPX-compressed
# Requires: UPX on PATH (https://upx.github.io/)
$ErrorActionPreference = "Stop"

Write-Host "[after-strip-upx] go build -ldflags='-s -w' -trimpath" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[after-strip-upx] upx --best --lzma app.exe" -ForegroundColor Cyan
