# after-minimize: Native AOT
# Requires: .NET SDK 8+ AND the C++ build tools (Visual Studio "Desktop development with C++" workload on Windows)
$ErrorActionPreference = "Stop"

Write-Host "[after-minimize] dotnet publish -c Release -r win-x64" -ForegroundColor Cyan
Write-Host "(this is a real native compile — expect 30s to 2min)" -ForegroundColor DarkGray
