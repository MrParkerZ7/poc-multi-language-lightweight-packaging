# 1-optimize-esbuild-llrt: esbuild bundle + AWS llrt binary (no Node install needed on host)
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[1-optimize-esbuild-llrt] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[1-optimize-esbuild-llrt] npm run build (esbuild for llrt)" -ForegroundColor Cyan
...
if (-not (Test-Path $llrtExe)) {
    ...
    Remove-Item $llrtZip
    if (-not (Test-Path $llrtExe)) {
        # zip layout sometimes differs â€” find any llrt*.exe
...
Write-Host ("TOTAL:     {0:N2} MB ({1:N0} bytes)  â€” ship both, no Node install needed" -f ($total / 1MB), $total)
