# 2-amalgamate (Node): esbuild max minify + llrt + UPX-compress llrt + scratch-ready
...
Write-Host "[2-amalgamate] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[2-amalgamate] esbuild bundle + max minify" -ForegroundColor Cyan
...
if (-not (Test-Path $llrtExe)) {
    Write-Host "[2-amalgamate] downloading AWS llrt" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[2-amalgamate] upx --best --lzma llrt.exe" -ForegroundColor Cyan
