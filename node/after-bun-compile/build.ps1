# after-bun-compile: bun build --compile — single binary with bun runtime bundled
...
Write-Host "[after-bun-compile] bun install (build-time only)" -ForegroundColor Cyan
bun install --silent | Out-Host

Write-Host "[after-bun-compile] bun build --compile --minify ..." -ForegroundColor Cyan
