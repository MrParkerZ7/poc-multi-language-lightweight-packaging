# 1-after-ncc: @vercel/ncc — zero-config bundler designed for Node services
# Simpler than webpack: one command, sane defaults, sourcemaps, minify.
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[1-after-ncc] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[1-after-ncc] npm run build (ncc)" -ForegroundColor Cyan
