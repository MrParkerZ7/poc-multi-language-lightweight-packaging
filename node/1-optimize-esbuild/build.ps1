# 1-optimize-esbuild: esbuild bundle + minify, single .mjs file (needs Node on host)
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[1-optimize-esbuild] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[1-optimize-esbuild] npm run build (esbuild)" -ForegroundColor Cyan
...
Write-Host ("Artifact: {0}  (single file, ship just this â€” Node on host runs it)" -f $artifact)
