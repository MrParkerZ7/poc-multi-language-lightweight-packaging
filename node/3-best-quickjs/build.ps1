$13-best-quickjs (Node): esbuild max minify + QuickJS-NG runtime (~1 MB) + UPX-LZMA + scratch-ready
...
Write-Host "[3-best-quickjs] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[3-best-quickjs] esbuild bundle + max minify (platform=neutral, target=es2023)" -ForegroundColor Cyan
npm run build | Out-Host

# Download QuickJS-NG release (precompiled binary)
$qjsExe = "dist/qjs.exe"
if (-not (Test-Path $qjsExe)) {
    Write-Host "[3-best-quickjs] downloading QuickJS-NG" -ForegroundColor Cyan
...
if ($upx -and (Test-Path $qjsExe)) {
    Write-Host "[3-best-quickjs] upx --best --lzma qjs.exe" -ForegroundColor Cyan
