# after-webpack: webpack bundle + Terser minify (industry-standard predecessor to esbuild)
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[after-webpack] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[after-webpack] npm run build (webpack)" -ForegroundColor Cyan
