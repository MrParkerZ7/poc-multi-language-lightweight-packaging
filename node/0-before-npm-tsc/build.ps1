# 0-before-npm-tsc: typical Node deployment â€” ship source + node_modules + dist
$ErrorActionPreference = "Stop"

if (Test-Path node_modules) { Remove-Item -Recurse -Force node_modules }
if (Test-Path dist)         { Remove-Item -Recurse -Force dist }

Write-Host "[0-before-npm-tsc] npm install" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[0-before-npm-tsc] npx tsc" -ForegroundColor Cyan
npx tsc | Out-Host
