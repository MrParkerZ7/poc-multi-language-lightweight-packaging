# after-esbuild-llrt: esbuild bundle + AWS llrt binary (no Node install needed on host)
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[after-esbuild-llrt] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[after-esbuild-llrt] npm run build (esbuild for llrt)" -ForegroundColor Cyan
