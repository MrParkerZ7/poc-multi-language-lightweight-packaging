# npm-tsc-before: typical Node deployment â€” ship source + node_modules + dist
$ErrorActionPreference = "Stop"

if (Test-Path node_modules) { Remove-Item -Recurse -Force node_modules }
if (Test-Path dist)         { Remove-Item -Recurse -Force dist }

Write-Host "[npm-tsc-before] npm install" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[npm-tsc-before] npx tsc" -ForegroundColor Cyan
npx tsc | Out-Host
