# 1-optimize-bun-compile: bun build --compile — single binary with bun runtime bundled
...
Write-Host "[1-optimize-bun-compile] bun install (build-time only)" -ForegroundColor Cyan
bun install --silent | Out-Host

Write-Host "[1-optimize-bun-compile] bun build --compile --minify ..." -ForegroundColor Cyan
