# after-esbuild: esbuild bundle + minify, single .mjs file (needs Node on host)
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[after-esbuild] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[after-esbuild] npm run build (esbuild)" -ForegroundColor Cyan
