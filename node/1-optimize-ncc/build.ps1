# 1-optimize-ncc: @vercel/ncc — zero-config bundler designed for Node services
# Simpler than webpack: one command, sane defaults, sourcemaps, minify.
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[1-optimize-ncc] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[1-optimize-ncc] npm run build (ncc)" -ForegroundColor Cyan
npm run build | Out-Host

$artifact = "dist/index.js"
if (-not (Test-Path $artifact)) { throw "ncc build failed: $artifact not found" }
