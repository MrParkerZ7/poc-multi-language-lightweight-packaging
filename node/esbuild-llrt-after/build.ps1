# esbuild-llrt-after: esbuild bundle + AWS llrt binary (no Node install needed on host)
$ErrorActionPreference = "Stop"

if (Test-Path dist) { Remove-Item -Recurse -Force dist }

Write-Host "[esbuild-llrt-after] npm install (build-time only)" -ForegroundColor Cyan
npm install --silent --no-audit --no-fund | Out-Host

Write-Host "[esbuild-llrt-after] npm run build (esbuild for llrt)" -ForegroundColor Cyan
