# after-minimize: aggressive size optimization + UPX
# Cargo.toml profile sets opt-level="z", lto, strip, panic=abort, codegen-units=1
$ErrorActionPreference = "Stop"

Write-Host "[after-minimize] cargo build --release  (size-tuned profile)" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[after-minimize] upx --best --lzma" -ForegroundColor Cyan
    upx --best --lzma $exe | Out-Host
    $compressedSize = (Get-Item $exe).Length
} else {
    Write-Warning "UPX not found on PATH — skipping compression. Install UPX for full lightweight result."
