# after-minimize-no-runtime: PyInstaller onefile — bundles a stripped Python interpreter
$ErrorActionPreference = "Stop"

# Make sure pyinstaller is available
$pi = (Get-Command pyinstaller -ErrorAction SilentlyContinue)
if (-not $pi) {
    Write-Host "[after-minimize-no-runtime] pip install --quiet pyinstaller" -ForegroundColor Cyan
...
Write-Host "[after-minimize-no-runtime] pyinstaller --onefile --strip --noconfirm app.py" -ForegroundColor Cyan
