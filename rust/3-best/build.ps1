# 3-best (Rust): nightly + build-std + panic_immediate_abort + optimize_for_size
...
Write-Host "[3-best] ensure nightly + rust-src + musl target" -ForegroundColor Cyan
rustup show active-toolchain 2>&1 | Out-Host

Write-Host "[3-best] cargo +nightly build --release --target x86_64-unknown-linux-musl" -ForegroundColor Cyan
