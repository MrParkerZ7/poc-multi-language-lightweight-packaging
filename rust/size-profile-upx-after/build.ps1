# size-profile-upx-after: aggressive size optimization + UPX
# Cargo.toml profile sets opt-level="z", lto, strip, panic=abort, codegen-units=1
$ErrorActionPreference = "Stop"

Write-Host "[size-profile-upx-after] cargo build --release  (size-tuned profile)" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[size-profile-upx-after] upx --best --lzma" -ForegroundColor Cyan
