# after-musl-static: statically-linked Linux binary (suitable for FROM scratch container)
...
Write-Host "[after-musl-static] rustup target add x86_64-unknown-linux-musl" -ForegroundColor Cyan
rustup target add x86_64-unknown-linux-musl 2>&1 | Out-Host

Write-Host "[after-musl-static] cargo build --release --target x86_64-unknown-linux-musl" -ForegroundColor Cyan
