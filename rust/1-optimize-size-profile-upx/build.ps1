# 1-optimize-size-profile-upx: aggressive size optimization + UPX
# Cargo.toml profile sets opt-level="z", lto, strip, panic=abort, codegen-units=1
$ErrorActionPreference = "Stop"

Write-Host "[1-optimize-size-profile-upx] cargo build --release  (size-tuned profile)" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[1-optimize-size-profile-upx] upx --best --lzma" -ForegroundColor Cyan
