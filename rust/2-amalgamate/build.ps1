# 2-amalgamate (Rust): musl static + opt-z + LTO=fat + strip + panic=abort + UPX + scratch
# The smallest reasonable Rust deployment shape — every applicable knob in the same direction.
$ErrorActionPreference = "Stop"

Write-Host "[2-amalgamate] rustup target add x86_64-unknown-linux-musl" -ForegroundColor Cyan
rustup target add x86_64-unknown-linux-musl 2>&1 | Out-Host

Write-Host "[2-amalgamate] cargo build --release --target x86_64-unknown-linux-musl" -ForegroundColor Cyan
...
if ($upx) {
    Write-Host "[2-amalgamate] upx --best --lzma" -ForegroundColor Cyan
