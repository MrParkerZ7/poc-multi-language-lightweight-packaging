...
| `after-minimize/` | jlink runtime image (folder) | ~32 MB | **No** (JRE bundled) | ~120 ms | Plain Java + Jackson, packaged with `jpackage --type app-image` (which uses jlink internally to ship only required JDK modules) |
| `after-minimize-no-runtime/` | GraalVM native image (single binary) | **~12 MB** | **No** | **~25 ms** | `native-image` ahead-of-time compilation via `native-maven-plugin` |
...
- Maven 3.9+
- For `after-minimize-no-runtime/`: GraalVM 21 with `native-image` (run `gu install native-image`)
