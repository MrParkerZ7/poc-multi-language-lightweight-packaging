# 1-optimize-pex: PEX (Python EXecutable) — Twitter/Pants's "zipapp on steroids"
...
if (-not $px) {
    Write-Host "[1-optimize-pex] pip install --quiet pex" -ForegroundColor Cyan
...
Write-Host "[1-optimize-pex] pex --output-file=dist/app.pex --entry-point=app:main ." -ForegroundColor Cyan
