$13-best-pyoxidizer (Python): PyOxidizer + memory-only modules + stripped CPython + UPX (Linux only)
...
if (-not $pyox) {
    Write-Host "[3-best-pyoxidizer] cargo install pyoxidizer" -ForegroundColor Cyan
...
Write-Host "[3-best-pyoxidizer] pyoxidizer build --release exe" -ForegroundColor Cyan
...
if ($upx -and $exe.Extension -ne ".exe") {
    Write-Host "[3-best-pyoxidizer] upx --best --lzma" -ForegroundColor Cyan
