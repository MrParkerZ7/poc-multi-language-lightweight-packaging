# after-nuitka: Nuitka — compile Python to C, then to a native binary (no Python on host)
...
if (-not $nu) {
    Write-Host "[after-nuitka] pip install --quiet nuitka" -ForegroundColor Cyan
...
Write-Host "[after-nuitka] python -m nuitka --onefile --output-dir=dist --remove-output app.py" -ForegroundColor Cyan
