# after-pex: PEX (Python EXecutable) — Twitter/Pants's "zipapp on steroids"
...
if (-not $px) {
    Write-Host "[after-pex] pip install --quiet pex" -ForegroundColor Cyan
...
Write-Host "[after-pex] pex --output-file=dist/app.pex --entry-point=app:main ." -ForegroundColor Cyan
