# 2-amalgamate (Python): Nuitka onefile + LTO + every size knob
...
if (-not $nu) {
    Write-Host "[2-amalgamate] pip install --quiet nuitka" -ForegroundColor Cyan
...
Write-Host "[2-amalgamate] python -m nuitka --onefile --lto=yes --no-pyi-file ..." -ForegroundColor Cyan
